...
{
    glconfig_t *glConfig = (glconfig_t*)windowData;
    if (windowData) {
        scale_ratio = (float)SCREEN_HEIGHT/glConfig->vidHeight;
        offset_x = (glConfig->vidWidth - ((float)SCREEN_WIDTH/scale_ratio))/2;
        Com_Printf("IN_Init: vidHeight=%d, vidWidth=%d, scale_ratio=%f, offset_x=%d\n",
                   glConfig->vidHeight, glConfig->vidWidth, scale_ratio, offset_x);
    } else {
        scale_ratio = 1;
        offset_x = 0;
        Com_Printf("IN_Init: no windowData received.\n");
    }
}
